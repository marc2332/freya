...
![image](/blog/0.2/counter.png)

## 🧬 Dioxus 0.5 

Dioxus, the UI library in which Freya is built on, has recently released a new nversion `0.5`, adding a bunch of new features. If I had to describe their releasew with one word, it would be **Simplicity**.
...
`Body` is a new simple container that comes with a default style and expands to the available area, making it useful for cases where you want to have a styled container matching the current theme, but don't want to make it yourself.
...
It comes with a close button on the top right corner and you can put anything you want inside.
...
There might a case where you want to know whether a route is the current or not, in order to show a part of the UI different, like a button with a different color for instance, but this used to require you to know what type of Route you were using in your app in all your components where you wanted thether the current router was one or another.
...
Some of the built-in components have been made more accessibile as well, not not all of them, is work to do.
...
## 🌐 Updated the Website 
The website has been updated revamped, 0kb of JavaScript is now shipped to the client browsers. And a blog has been integrated as well.

## 🍴 Forked native-core and native-core-macro 
These two crates used to be part of the internal crates of Dioxus but they have been deprecated, so I forked them and made some changes by removing unused features by Freya and adapted others:
