...
Freya now comes with a Plugin API to bring extra features to apps, it consists of a simple Trait and a register method.
...
impl FreyaPlugin for DummyPlugin {
    fn on_event(&mut self, event: &PluginEvent) {
        // You can react on certain events like Rendering
...
fn main() {
    launch_cfg(
        ...
        LaunchConfig::<()>::builder()
            .with_plugin(DummyPlugin) // Here you register the instance of the plugin
            .with_width(250.0)
            .with_height(200.0)
            .build(),
    )
...
Freya now comes with a Performance plugin you can register to test the performance of your app.

It displays a few metrics:

- FPS: Amount of frames rendered in the last second, this will only really matter if you are actually rerendering a lot, otherwise it's kinda useless as it will show low FPS and it wouldn't be a bad thing.
- Rendering: Time it took to render.
- Layout: Time it took to measure the layout.
- DOM Updates: Time it took to sinchronize the Native DOM with the changes from the VirtualDOM.
- DOM Nodes: Amount of nodes in the Native DOM.
- Layout Nodes: Amount of nodes in the Layout tree, text nodes are not standalone nodes hence why Layout Nodes will be lower than DOM Nodes most of the time.
