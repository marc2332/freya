use freya_node_state::{Parse, LinearGradient, GradientStop};
use skia_safe::Color;

#[test]
fn parse_basic_gradient() {
    assert_eq!(LinearGradient::parse("linear-gradient(red 0%, blue 100%)"), Ok(LinearGradient {
		angle: 0.0,
		stops: vec![
			GradientStop {
				color: Color::RED,
...
fn parse_rgb_hsl_gradient() {
    assert_eq!(LinearGradient::parse("linear-gradient(0deg, rgb(255, 0, 0) 0%, blue 100%)"), Ok(LinearGradient {
		angle: 0.0,
		stops: vec![
			GradientStop {
...
fn parse_gradient_angle() {
    assert_eq!(LinearGradient::parse("linear-gradient(45deg, red 0%, blue 100%)"), Ok(LinearGradient {
