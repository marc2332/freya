...
fn app(cx: Scope) -> Element {
    ...
    render!(
        rect {
            ...
            label { font_size: "75", font_weight: "bold", "Background brightness: {brightness}. Text brightness: {text_brightness}" }
        }
        rect { height: "50%", width: "100%", main_align: "center", cross_align: "center", direction: "horizontal",
            Button {
                ...
                onclick: move |_| if *brightness < 100 { brightness += 10 },
                label { "Increase background brightness" }
            }
            Button {
                theme: theme_with!(ButtonTheme {
                ...
                onclick: move |_| if *brightness > 0 { brightness -= 10 },
                label { "Decrease background brightness" }
            }
        }
    )
}
