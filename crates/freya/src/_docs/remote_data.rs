//! # Remote Data
//!
//! Freya Query provides async data management for Freya applications,
//! handling caching, background refetching, deduplication, and automatic invalidation.
//!
...
//! ```
//!
//! It manages two types of operations:
//!
//! - **Queries** ([`use_query`](crate::query::use_query)): Read operations that fetch, cache, and reactively share data.
//! - **Mutations** ([`use_mutation`](crate::query::use_mutation)): Write operations that modify data and can invalidate queries.
//!
//! ## Queries
//!
//! ### Defining a query
//!
//! Implement [`QueryCapability`](crate::query::QueryCapability) on a type to define how data is fetched:
//!
//! ```rust,no_run
//! # use freya::prelude::*;
//! # use freya::query::*;
...
//!
//! // Await the result
//! let reader = user.invalidate_async().await;
//! ```
//!
...
//! ### Defining a mutation
//!
//! Implement [`MutationCapability`](crate::query::MutationCapability) to define a write operation:
//!
//! ```rust,no_run
//! # use freya::prelude::*;
//! # use freya::query::*;
...
//! ```
//!
//! The [`on_settled`](crate::query::MutationCapability::on_settled) callback is the primary mechanism for
...
//!
//! Call [`use_mutation`](crate::query::use_mutation) with a [`Mutation`](crate::query::Mutation):
//!
//! ```rust,no_run
//! # use freya::prelude::*;
//! # use freya::query::*;
...
//! ```
//!
//! See [`UseMutation`](crate::query::UseMutation) docs for the full API. The state is exposed
//! as [`MutationStateData`](crate::query::MutationStateData).
//!
//! [`Mutation`](crate::query::Mutation) also supports builder methods, see its docs.
//!
//! ## Captured values
//!
...
//!
//! [`Captured<T>`](crate::query::Captured) wraps a value so it is invisible to caching:
...
//! value transparently.
//!
//! ## Full example: queries + mutations
//!
//! ```rust,no_run
//! use std::time::Duration;
//!
...
//! }
//!
//! // Mutation: update the user name, then invalidate the query
//! #[derive(Clone, PartialEq, Hash, Eq)]
