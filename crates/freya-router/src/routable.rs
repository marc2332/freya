...
/// Formats a value as one or more URL path segments.
///
/// This is used by `#[derive(Routable)]` for catch-all route fields.
...
pub trait Routable: FromStr<Err: Display> + Display + Clone + 'static {
    ...
    /// Renders the route at `level` in the nested route hierarchy.
    ///
    /// Implementations are normally generated by `#[derive(Routable)]`.
    fn render(&self, level: usize) -> Element;

    /// Returns whether this route is nested below `other`.
    ///
    /// Query parameters and hash fragments do not affect the result.
    ...
    /// Returns this route's parent, if it can be parsed as the same route type.
    ...
    /// Returns every route pattern in the generated site map as path segments.
    fn flatten_site_map<'a>() -> SiteMapFlattened<'a> {
        Self::SITE_MAP.iter().flat_map(SiteMapSegment::flatten)
    }

    /// Returns all routes that have no dynamic or catch-all segments.
    ///
    /// For example, `#[route("/static/route")]` is static.
...
impl SiteMapSegment {
    /// Flattens this branch of the site map into complete route patterns.
